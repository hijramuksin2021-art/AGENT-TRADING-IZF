...
type AutoTraderConfig struct {
	...
	// MT5 configuration
	MT5BridgeURL string // MT5 Python bridge URL (default: http://localhost:5555)
...
func NewAutoTrader(config AutoTraderConfig, st *store.Store, userID string) (*AutoTrader, error) {
	...
	case "mt5":
		logger.Infof("🏦 [%s] Using MetaTrader 5 Forex trading", config.Name)
		bridgeURL := config.MT5BridgeURL
		if bridgeURL == "" {
			bridgeURL = "http://localhost:5555"
		}
		mt5Trader := mt5.NewMT5Trader(bridgeURL)
		// Test connection at startup
		if _, connErr := mt5Trader.TestConnection(); connErr != nil {
			logger.Warnf("⚠️ [%s] MT5 bridge not reachable: %v (will retry when trading)", config.Name, connErr)
		}
		trader = mt5Trader
	default:
