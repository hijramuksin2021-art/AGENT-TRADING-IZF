...
func (at *AutoTrader) buildTradingContext() (*kernel.Context, error) {
	...
	// Detect MT5 forex mode (used for symbol fetching, data skip, and prompt adaptation)
	isMT5 := strings.ToLower(at.config.Exchange) == "mt5"
	...
	} else if strings.ToLower(at.config.Exchange) == "mt5" {
		// MT5 forex: use user-selected symbols or fetch from bridge
		stratCfg := at.strategyEngine.GetConfig()

		// If user has manually picked symbols in strategy config, use those directly
		if stratCfg.CoinSource.SourceType == "mt5_symbols" && len(stratCfg.CoinSource.StaticCoins) > 0 {
			at.logInfof("💱 MT5: using %d user-selected symbols", len(stratCfg.CoinSource.StaticCoins))
			for _, sym := range stratCfg.CoinSource.StaticCoins {
				candidateCoins = append(candidateCoins, kernel.CandidateCoin{
					Symbol:  sym,
					Sources: []string{"mt5_user_selected"},
				})
			}
		} else {
			// Fallback: auto-fetch from MT5 bridge
			at.logInfof("💱 MT5 exchange detected, fetching forex symbols from bridge...")
			category := stratCfg.CoinSource.MT5Category
			if category == "" {
				category = "forex"
			}
			limit := stratCfg.CoinSource.MT5Limit
			if limit <= 0 {
				limit = 10
			}
			// Temporarily override source type for this cycle
			origSourceType := stratCfg.CoinSource.SourceType
			stratCfg.CoinSource.SourceType = "mt5_symbols"
			stratCfg.CoinSource.MT5Category = category
			stratCfg.CoinSource.MT5Limit = limit

			coins, err := at.strategyEngine.GetCandidateCoins()
			// Restore original source type
			stratCfg.CoinSource.SourceType = origSourceType

			if err != nil {
				at.logWarnf("⚠️ Failed to get MT5 symbols: %v (will use empty list)", err)
			} else {
				candidateCoins = coins
				logger.Infof("💱 [%s] MT5 bridge fetched %d forex symbols", at.name, len(candidateCoins))
			}
		}
	} else {
		coins, err := at.strategyEngine.GetCandidateCoins()
	...
	ctx := &kernel.Context{
		...
		MarketType:      map[bool]string{true: "forex", false: "crypto"}[isMT5],
	...
	// 8-11. Skip crypto-specific data for MT5 forex (OI, NetFlow, QuantData are crypto-only)

	// 8. Get quantitative data (if enabled in strategy config)
	if strategyConfig.Indicators.EnableQuantData && !isMT5 {
	...
	// 9. Get OI ranking data (market-wide position changes) - crypto only
	if strategyConfig.Indicators.EnableOIRanking && !isMT5 {
	...
	// 10. Get NetFlow ranking data (market-wide fund flow) - crypto only
	if strategyConfig.Indicators.EnableNetFlowRanking && !isMT5 {
	...
	// 11. Get Price ranking data (market-wide gainers/losers) - crypto only
	if strategyConfig.Indicators.EnablePriceRanking && !isMT5 {
