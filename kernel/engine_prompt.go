...
func (e *StrategyEngine) BuildSystemPrompt(accountEquity float64, variant string, marketType ...string) string {
	...
	if len(marketType) > 0 && marketType[0] != "" {
		mktType = marketType[0]
	}
	isForex := mktType == "forex"
	...
	// 1. Role definition (editable) — override for forex
	if isForex {
		if promptSections.RoleDefinition != "" && !strings.Contains(promptSections.RoleDefinition, "cryptocurrency") {
			sb.WriteString(promptSections.RoleDefinition)
		} else {
			sb.WriteString("# You are a professional Forex & CFD trading AI\n\n")
			sb.WriteString("Your task is to make trading decisions on currency pairs, metals, and indices based on provided market data.\n")
			sb.WriteString("You are an experienced forex trader skilled in technical analysis, price action, support/resistance levels, and risk management.\n\n")
			sb.WriteString("## Forex Market Context\n")
			sb.WriteString("- You trade currency pairs (EURUSD, GBPUSD, USDJPY, etc.), metals (XAUUSD, XAGUSD), and indices\n")
			sb.WriteString("- Forex markets operate 24/5, liquidity varies by session (London, New York, Tokyo, Sydney)\n")
			sb.WriteString("- Key drivers: central bank policies, economic data releases, geopolitical events, risk sentiment\n")
			sb.WriteString("- Spreads and pip values matter — consider transaction costs in your analysis\n\n")
		}
	...
	if isForex {
		// Forex: use single position value ratio (use the higher btcEth ratio for forex majors)
		sb.WriteString(fmt.Sprintf("- Max Position Value: %.0f (= equity × %.1fx)\n",
			accountEquity*btcEthPosValueRatio, btcEthPosValueRatio))
	} else {
	...
	sb.WriteString(fmt.Sprintf("- Min Position Size: ≥%.0f\n\n", riskControl.MinPositionSize))
	...
	if promptSections.DecisionProcess != "" {
		...
		if isForex {
			sb.WriteString("2. Scan currency pairs + multi-timeframe → Are there strong signals\n")
		} else {
...
func (e *StrategyEngine) BuildUserPrompt(ctx *Context) string {
	...
	isForex := ctx.MarketType == "forex"

	// BTC market (crypto only)
	if !isForex {
	...
	if isForex {
		sb.WriteString(fmt.Sprintf("## Currency Pairs (%d symbols)\n\n", len(ctx.MarketDataMap)))
	} else {
