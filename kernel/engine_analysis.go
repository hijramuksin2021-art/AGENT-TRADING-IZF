...
func GetFullDecisionWithStrategy(ctx *Context, mcpClient mcp.AIClient, engine *StrategyEngine, variant string) (*FullDecision, error) {
	...
	// Ensure OITopDataMap is initialized (crypto only — forex has no OI data)
	if ctx.OITopDataMap == nil && ctx.MarketType != "forex" {
	...
	systemPrompt := engine.BuildSystemPrompt(ctx.Account.TotalEquity, variant, ctx.MarketType)
...
func fetchMarketDataWithStrategy(ctx *Context, engine *StrategyEngine) error {
	...
	if len(timeframes) == 0 {
		if primaryTimeframe != "" {
			...
			timeframes = append(timeframes, "5m") // Forex default: 5m instead of 3m
		} else {
		...
		if config.Indicators.Klines.LongerTimeframe != "" {
			...
			timeframes = append(timeframes, "1h") // Forex default longer timeframe
		}
	}
	if primaryTimeframe == "" {
	...
	logger.Infof("📊 Strategy timeframes: %v, Primary: %s, Kline count: %d, Market: %s",
		timeframes, primaryTimeframe, klineCount, ctx.MarketType)

	// Select data fetch function based on market type
	fetchData := func(symbol string) (*market.Data, error) {
		if isForex {
			return market.GetFromMT5Bridge(symbol, timeframes, primaryTimeframe, klineCount)
		}
	...
	for _, coin := range ctx.CandidateCoins {
		...
		data, err := fetchData(coin.Symbol)
		if err != nil {
			logger.Infof("⚠️  Failed to fetch market data for %s: %v", coin.Symbol, err)
			continue
		}

		// Liquidity filter (skip for xyz dex assets and forex - they don't have OI data from Binance)
		isExistingPosition := positionSymbols[coin.Symbol]
		isXyzAsset := market.IsXyzDexAsset(coin.Symbol)
		if !isExistingPosition && !isXyzAsset && !isForex && data.OpenInterest != nil && data.CurrentPrice > 0 {
	...
	logger.Infof("📊 Successfully fetched multi-timeframe market data for %d symbols", len(ctx.MarketDataMap))
	return nil
}
...
func parseFullDecisionResponse(aiResponse string, accountEquity float64, btcEthLeverage, altcoinLeverage int, btcEthPosRatio, altcoinPosRatio float64, marketType string) (*FullDecision, error) {
	...
	if err := validateDecisions(decisions, accountEquity, btcEthLeverage, altcoinLeverage, btcEthPosRatio, altcoinPosRatio, marketType); err != nil {
		return &FullDecision{
			CoTTrace:  cotTrace,
			Decisions: decisions,
