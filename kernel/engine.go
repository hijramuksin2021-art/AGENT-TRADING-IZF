...
type Context struct {
	...
	MarketType         string                             `json:"-"` // "crypto" (default) or "forex"
...
func (e *StrategyEngine) GetCandidateCoins() ([]CandidateCoin, error) {
	...
	case "mt5_symbols":
		// Fetch symbols from MT5 bridge (forex, metals, indices, etc.)
		coins, err := e.getMT5Symbols(coinSource.MT5Category, coinSource.MT5Limit)
		if err != nil {
			// Fall back to static coins if bridge unavailable
			logger.Infof("⚠️  MT5 bridge unavailable, falling back to static coins: %v", err)
			for _, symbol := range coinSource.StaticCoins {
				candidates = append(candidates, CandidateCoin{
					Symbol:  symbol,
					Sources: []string{"static"},
...
// getMT5Symbols fetches available trading symbols from MT5 bridge
func (e *StrategyEngine) getMT5Symbols(category string, limit int) ([]CandidateCoin, error) {
	if limit <= 0 {
		limit = 20 // Default: top 20 forex pairs
	}
	if category == "" {
		category = "forex" // Default to forex category
	}
	...
	if err != nil {
		return nil, fmt.Errorf("MT5 bridge connection failed: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("MT5 bridge returned status %d", resp.StatusCode)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return nil, fmt.Errorf("failed to parse MT5 symbols: %w", err)
	}
	...
	for i, sym := range result.Symbols {
		...
		candidates = append(candidates, CandidateCoin{
			Symbol:  sym.Name,
			Sources: []string{"mt5_" + sym.Category},
		})
	}

	logger.Infof("✅ Loaded %d MT5 %s symbols from bridge", len(candidates), category)
	return candidates, nil
}
