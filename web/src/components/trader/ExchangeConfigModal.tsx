...
const SUPPORTED_EXCHANGE_TEMPLATES = [
  ...
  { exchange_type: 'mt5', name: 'MetaTrader 5', type: 'forex' as const },
]
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{
          background: template.type === 'cex' ? 'rgba(240, 185, 11, 0.2)'
            : template.type === 'forex' ? 'rgba(34, 197, 94, 0.2)'
            : 'rgba(139, 92, 246, 0.2)',
          color: template.type === 'cex' ? '#F0B90B'
            : template.type === 'forex' ? '#22C55E'
            : '#A78BFA',
        }}
      >
...
export function ExchangeConfigModal({
  ...
  // MT5 fields
  const [mt5Status, setMt5Status] = useState<'idle' | 'testing' | 'connected' | 'failed'>('idle')
  const [mt5Info, setMt5Info] = useState<string>('')
  const [mt5BridgeUrl, setMt5BridgeUrl] = useState('http://localhost:5555')
  ...
  const exchangeRegistrationLinks: Record<string, { url: string; hasReferral?: boolean }> = {
    ...
    mt5: { url: 'https://www.metatrader5.com/en/download', hasReferral: false },
  }
  ...
  useEffect(() => {
    if (editingExchangeId && selectedExchange) {
      ...
      // Restore saved bridge URL for MT5 (stored in secretKey field)
      if (selectedExchange.exchange_type === 'mt5' && selectedExchange.apiKey && selectedExchange.apiKey !== 'mt5-local') {
        setMt5BridgeUrl(selectedExchange.apiKey)
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (currentExchangeType === 'mt5') {
        // Save bridge URL in the apiKey field so backend knows where to connect
        const bridgeUrl = mt5BridgeUrl.trim() || 'http://localhost:5555'
        await onSave(exchangeId, exchangeType, trimmedAccountName, bridgeUrl, 'mt5-bridge', '', false)
      } else {
        if (!apiKey.trim() || !secretKey.trim()) return
        await onSave(exchangeId, exchangeType, trimmedAccountName, apiKey.trim(), secretKey.trim(), '', testnet)
      }
  ...
  const forexExchanges = SUPPORTED_EXCHANGE_TEMPLATES.filter(t => t.type === 'forex')
                ...
                <>
                  <div className="p-4 rounded-xl" style={{ background: 'rgba(34, 197, 94, 0.1)', border: '1px solid rgba(34, 197, 94, 0.3)' }}>
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs" style={{ color: '#848E9C' }}>
                          Jalankan MT5 Bridge (<code style={{ color: '#22C55E' }}>python server.py</code>) di PC lokal kamu yang sudah terbuka MT5-nya, lalu masukkan URL bridge di bawah ini.
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 text-sm font-semibold" style={{ color: '#EAECEF' }}>
                      ...
                      MT5 Bridge URL
                      <Tooltip content="Alamat Python bridge server yang menghubungkan IZF ke MT5 di PC kamu. Default: http://localhost:5555. Jika IZF diakses dari perangkat lain, gunakan IP LAN kamu, contoh: http://192.168.1.10:5555">
                        <HelpCircle className="w-4 h-4 cursor-help" style={{ color: '#22C55E' }} />
                      </Tooltip>
                    </label>
                    <input
                      type="text"
                      ...
                      style={{ background: '#0B0E11', border: '1px solid #2B3139', color: '#22C55E' }}
                    />
                    <div className="text-xs" style={{ color: '#5E6673' }}>
                      💡 Akses dari PC sendiri: <code style={{ color: '#22C55E' }}>http://localhost:5555</code> · Akses dari HP/device lain di jaringan sama: <code style={{ color: '#22C55E' }}>http://192.168.x.x:5555</code>
                    </div>
                  </div>

                  {/* Connection Test */}
                  <div className="p-4 rounded-xl space-y-3" style={{ background: '#0B0E11', border: '1px solid #2B3139' }}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full" style={{ background: mt5Status === 'connected' ? '#0ECB81' : mt5Status === 'failed' ? '#F6465D' : mt5Status === 'testing' ? '#F0B90B' : '#2B3139', boxShadow: mt5Status === 'connected' ? '0 0 8px rgba(14, 203, 129, 0.5)' : 'none' }} />
                        <span className="text-sm font-medium" style={{ color: mt5Status === 'connected' ? '#0ECB81' : mt5Status === 'failed' ? '#F6465D' : '#848E9C' }}>
                          {mt5Status === 'connected' ? '✅ Terhubung ke MT5' : mt5Status === 'failed' ? '❌ Gagal terhubung' : mt5Status === 'testing' ? '⏳ Menghubungkan...' : 'Belum terhubung'}
                        </span>
                      </div>
                      <button
                        type="button"
                        ...
                        onClick={async () => {
                          ...
                          setMt5Info('')
                          const targetUrl = (mt5BridgeUrl.trim() || 'http://localhost:5555').replace(/\/$/, '')
                          try {
                            ...
                            if (data.connected) {
                              ...
                              if (acct) {
                                setMt5Info(`${acct.trade_mode?.toUpperCase()} | Login: ${acct.login} | Balance: ${acct.balance} ${acct.currency} | Leverage: 1:${acct.leverage}`)
                                if (!accountName) setAccountName(`MT5-${acct.login}`)
                              }
                            } else {
                              setMt5Status('failed')
                              setMt5Info(data.error || 'MT5 tidak terhubung')
                            }
                          } catch {
                            setMt5Status('failed')
                            setMt5Info(`MT5 Bridge tidak ditemukan di ${targetUrl}. Pastikan sudah menjalankan: python server.py`)
                          }
                        }}
                        className="px-4 py-2 rounded-lg text-sm font-semibold transition-all hover:scale-105 disabled:opacity-50"
                        style={{ background: '#22C55E', color: '#000' }}
                      >
                        {mt5Status === 'testing' ? '⏳ Testing...' : '🔗 Test Connection'}
                      </button>
                    </div>
                    {mt5Info && (
                      <div className="text-xs px-3 py-2 rounded-lg" style={{ background: mt5Status === 'connected' ? 'rgba(14, 203, 129, 0.1)' : 'rgba(246, 70, 93, 0.1)', color: mt5Status === 'connected' ? '#9FE8C5' : '#F6465D', border: mt5Status === 'connected' ? '1px solid rgba(14, 203, 129, 0.2)' : '1px solid rgba(246, 70, 93, 0.2)' }}>
                        {mt5Info}
                      </div>
                    )}
                  </div>

                  <div className="p-3 rounded-xl text-xs" style={{ background: 'rgba(240, 185, 11, 0.08)', border: '1px solid rgba(240, 185, 11, 0.2)', color: '#F0B90B' }}>
                    💡 <strong>Urutan startup:</strong> Buka MT5 Terminal → jalankan <code>python server.py</code> → isi Bridge URL di atas → klik Test Connection
                  </div>
                </>
              )}
