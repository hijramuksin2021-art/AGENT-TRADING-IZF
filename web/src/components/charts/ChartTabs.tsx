...
const MARKET_CONFIG = {
  ...
  forex: { exchange: 'mt5', defaultSymbol: 'EURUSD', icon: '💱', labelKey: 'forex' as const, color: 'blue', hasDropdown: true },
  metals: { exchange: 'mt5', defaultSymbol: 'XAUUSD', icon: '🥇', labelKey: 'metals' as const, color: 'amber', hasDropdown: true },
}
...
function getMarketTypeFromExchange(exchangeId: string | undefined): MarketType {
  ...
  if (lower.includes('mt5') || lower.includes('metatrader')) return 'forex'
...
export function ChartTabs({ traderId, selectedSymbol, updateKey, exchangeId }: ChartTabsProps) {
  ...
  useEffect(() => {
    if (marketConfig.hasDropdown) {
      const fetchUrl = marketConfig.exchange === 'mt5'
        ? `/api/symbols?exchange=mt5`
        : `/api/symbols?exchange=${marketConfig.exchange}`
      fetch(fetchUrl)
        .then(res => res.json())
        .then(data => {
          if (data.symbols) {
                  ...
                  {showDropdown && (
                    <div className="absolute top-full right-0 mt-2 w-64 bg-[#0B0E11] border border-white/10 rounded-lg shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] z-50 overflow-hidden nofx-glass ring-1 ring-white/5">
                      ...
                      <div className="overflow-y-auto max-h-60 custom-scrollbar">
                        {['crypto', 'forex', 'metals', 'stock', 'indices', 'energy', 'commodity', 'index', 'other'].map(category => {
                          const categorySymbols = filteredSymbols.filter(s => s.category === category)
                          if (categorySymbols.length === 0) return null
                          const labels: Record<string, string> = { crypto: 'Crypto', stock: 'Stocks', forex: 'Forex', metals: 'Metals', commodity: 'Commodities', index: 'Index', indices: 'Indices', energy: 'Energy', other: 'Other' }
                          return (
                            <div key={category}>
