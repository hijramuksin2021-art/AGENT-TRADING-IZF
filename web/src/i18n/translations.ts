...
export const translations = {
  en: {
    ...
    // Footer
    footerTitle: 'IZF - AI Trading System',
    ...
    heroDescription:
      'IZF is the future standard for AI trading — an open, community-driven agentic trading OS. Supporting Binance, Aster DEX and other exchanges, self-hosted, multi-agent competition, let AI automatically make decisions, execute and optimize trades for you.',
    poweredBy: 'Powered by Aster DEX and Binance.',

    // Landing Page CTA
    readyToDefine: 'Ready to define the future of AI trading?',
    startWithCrypto:
      'Starting with crypto markets, expanding to TradFi. IZF is the infrastructure of AgentFi.',
    getStartedNow: 'Get Started Now',
    viewSourceCode: 'View Source Code',

    // Features Section
    coreFeatures: 'Core Features',
    whyChooseIZF: 'Why Choose IZF?',
    openCommunityDriven:
    ...
    // About Section
    aboutIZF: 'About IZF',
    whatIsIZF: 'What is IZF?',
    IZFNotAnotherBot:
      "IZF is not another trading bot, but the 'Linux' of AI trading —",
    IZFDescription1:
    ...
    howToStart: 'How to Get Started with IZF',
    fourSimpleSteps:
      'Four simple steps to start your AI automated trading journey',
    step1Title: 'Clone GitHub Repository',
    step1Desc:
      'git clone https://github.com/IZFAiOS/IZF and switch to dev branch to test new features.',
    ...
    riskWarningText:
      'Dev branch is unstable, do not use funds you cannot afford to lose. IZF is non-custodial, no official strategies. Trading involves risks, invest carefully.',
    ...
    // Login Modal
    accessIZFPlatform: 'Access IZF Platform',
    loginRegisterPrompt:
    ...
    faqSubtitle: 'Find answers to common questions about IZF',
    ...
    // ===== GETTING STARTED =====
    faqWhatIsIZF: 'What is IZF?',
    faqWhatIsIZFAnswer:
      'IZF is an open-source AI-powered trading operating system for cryptocurrency and US stock markets. It uses large language models (LLMs) like DeepSeek, GPT, Claude, Gemini to analyze market data and make autonomous trading decisions. Key features include: multi-AI model support, multi-exchange trading, and visual strategy builder.',

    faqHowDoesItWork: 'How does IZF work?',
    faqHowDoesItWorkAnswer:
      'IZF works in 5 steps: 1) Configure AI models and exchange API credentials; 2) Create a trading strategy (coin selection, indicators, risk controls); 3) Create a "Trader" combining AI model + Exchange + Strategy; 4) Start the trader - it will analyze market data at regular intervals and make buy/sell/hold decisions; 5) Monitor performance on the dashboard. The AI uses Chain of Thought reasoning to explain each decision.',

    faqIsProfitable: 'Is IZF profitable?',
    faqIsProfitableAnswer:
      'AI trading is experimental and NOT guaranteed to be profitable. Cryptocurrency futures are highly volatile and risky. IZF is designed for educational and research purposes. We strongly recommend: starting with small amounts (10-50 USDT), never investing more than you can afford to lose, thoroughly testing before live trading, and understanding that past performance does not guarantee future results.',
    ...
    // ===== INSTALLATION =====
    faqHowToInstall: 'How do I install IZF?',
    faqHowToInstallAnswer:
      'Easiest method (Linux/macOS): Run "curl -fsSL https://raw.githubusercontent.com/IZFAiOS/IZF/main/install.sh | bash" - this installs Docker containers automatically. Then open http://127.0.0.1:3000 in your browser. For manual installation or development, clone the repository and follow the README instructions.',
    ...
    faqManualInstallationAnswer:
      'Prerequisites: Go 1.21+, Node.js 18+, TA-Lib. Steps: 1) Clone repo: "git clone https://github.com/IZFAiOS/IZF.git"; 2) Install backend deps: "go mod download"; 3) Install frontend deps: "cd web && npm install"; 4) Build backend: "go build -o IZF"; 5) Run backend: "./IZF"; 6) Run frontend (new terminal): "cd web && npm run dev". Access at http://127.0.0.1:3000',
    ...
    faqUpdateIZF: 'How do I update IZF?',
    faqUpdateIZFAnswer:
      'For Docker: Run "docker compose pull && docker compose up -d" to pull latest images and restart. For manual installation: "git pull && go build -o IZF" for backend, "cd web && npm install && npm run build" for frontend. Your configurations in data.db are preserved during updates.',
    ...
    faqStopLossTakeProfit: 'Does IZF support stop-loss and take-profit?',
    ...
    faqMultipleTradersAnswer:
      'Yes! IZF supports running 20+ concurrent traders. Each trader can have different: AI model, exchange account, strategy, decision interval. Use this to A/B test strategies, compare AI models, or diversify across exchanges. Monitor all traders on the Competition page.',
    ...
    faqPortInUseAnswer:
      'Check what\'s using the port: "lsof -i :8080" (macOS/Linux) or "netstat -ano | findstr 8080" (Windows). Kill the process or change ports in .env: IZF_BACKEND_PORT=8081, IZF_FRONTEND_PORT=3001. Restart with "docker compose down && docker compose up -d".',

    faqFrontendNotLoading: 'Frontend shows "Loading..." forever',
    faqFrontendNotLoadingAnswer:
      'Backend may not be running or reachable. Check: 1) "curl http://127.0.0.1:8080/api/health" should return {"status":"ok"}; 2) "docker compose ps" to verify containers are running; 3) Check backend logs: "docker compose logs IZF-backend"; 4) Ensure firewall allows port 8080.',

    faqDatabaseLocked: 'Database locked error',
    faqDatabaseLockedAnswer:
      'Multiple processes accessing SQLite simultaneously. Solution: 1) Stop all processes: "docker compose down" or "pkill IZF"; 2) Remove lock files if present: "rm -f data/data.db-wal data/data.db-shm"; 3) Restart: "docker compose up -d". Only one backend instance should access the database.',

    faqTALibNotFound: 'TA-Lib not found during build',
    faqTALibNotFoundAnswer:
      'TA-Lib is required for technical indicators. Install: macOS: "brew install ta-lib"; Ubuntu/Debian: "sudo apt-get install libta-lib0-dev"; CentOS: "yum install ta-lib-devel". After installing, rebuild: "go build -o IZF". Docker images include TA-Lib pre-installed.',
    ...
    faqBinancePositionModeAnswer:
      'Error: "Order\'s position side does not match user\'s setting". You\'re in One-way Mode but IZF requires Hedge Mode. Fix: 1) Close ALL positions first; 2) Binance Futures → Settings (gear icon) → Preferences → Position Mode → Switch to "Hedge Mode" (双向持仓); 3) Restart your trader.',

    faqBalanceShowsZero: 'Account balance shows 0',
    faqBalanceShowsZeroAnswer:
      'Funds are likely in Spot wallet, not Futures wallet. Solution: 1) In Binance, go to Wallet → Futures → Transfer; 2) Transfer USDT from Spot to Futures; 3) Refresh IZF dashboard. Also check: funds not locked in savings/staking products.',
    ...
    faqEncryptionDetails: 'What encryption does IZF use?',
    faqEncryptionDetailsAnswer:
      'IZF uses multiple encryption layers: 1) AES-256-GCM for database storage (API keys, secrets); 2) RSA-2048 for optional transport encryption (browser to server); 3) JWT for authentication tokens. Keys are generated during installation. Enable TRANSPORT_ENCRYPTION=true for HTTPS environments.',

    faqSecurityBestPractices: 'What are security best practices?',
    faqSecurityBestPracticesAnswer:
      'Recommended: 1) Use exchange API keys with IP whitelist and minimal permissions (Futures Trading only); 2) Use dedicated subaccount for IZF; 3) Enable TRANSPORT_ENCRYPTION for remote deployments; 4) Never share .env or data.db files; 5) Use HTTPS with valid certificates; 6) Regularly rotate API keys; 7) Monitor account activity.',

    faqCanIZFStealFunds: 'Can IZF steal my funds?',
    faqCanIZFStealFundsAnswer:
      'IZF is open-source (AGPL-3.0 license) - you can audit all code on GitHub. API keys are stored locally on YOUR machine, never sent to external servers. IZF only has the permissions you grant via API keys. For maximum safety: use API keys with trading-only permissions (no withdrawal), enable IP whitelist, use a dedicated subaccount.',
    ...
    faqCustomAIAPIAnswer:
      'Yes! IZF supports any OpenAI-compatible API. In Config → AI Models → Custom API: 1) Enter your API endpoint URL (e.g., https://your-api.com/v1); 2) Enter API key; 3) Specify model name. This works with self-hosted models, alternative providers, or Claude via third-party proxies.',

    faqAIHallucinations: 'What about AI hallucinations?',
    faqAIHallucinationsAnswer:
      'AI models can sometimes produce incorrect or fabricated information ("hallucinations"). IZF mitigates this by: 1) Providing structured prompts with real market data; 2) Enforcing JSON output format for decisions; 3) Validating orders before execution. However, AI trading is experimental - always monitor decisions and don\'t rely solely on AI judgment.',
    ...
    // ===== CONTRIBUTING =====
    faqHowToContribute: 'How can I contribute to IZF?',
    faqHowToContributeAnswer:
      'IZF is open-source and welcomes contributions! Ways to contribute: 1) Code - fix bugs, add features (check GitHub Issues); 2) Documentation - improve guides, translate; 3) Bug Reports - report issues with details; 4) Feature Ideas - suggest improvements. Start with issues labeled "good first issue". All contributors may receive airdrop rewards.',

    faqPRGuidelines: 'What are the PR guidelines?',
    faqPRGuidelinesAnswer:
      'PR Process: 1) Fork repo to your account; 2) Create feature branch from dev: "git checkout -b feat/your-feature"; 3) Make changes, run lint: "npm --prefix web run lint"; 4) Commit with Conventional Commits format; 5) Push and create PR to IZFAiOS/IZF:dev; 6) Reference related issue (Closes #123); 7) Wait for review. Keep PRs small and focused.',
    ...
    environmentCheck: {
      ...
      unsupportedDesc:
        'Open IZF over HTTPS (or http://localhost during development) and avoid insecure iframes/reverse proxies so the browser can enable Web Crypto.',
  ...
  zh: {
    ...
    heroTitle2: 'Write the Trade.',
    heroDescription:
      'IZF 是 AI 交易的未来标准——一个开放、社区驱动的代理式交易操作系统。支持 Binance、Aster DEX 等交易所，自托管、多代理竞争，让 AI 为你自动决策、执行和优化交易。',
    poweredBy: '由 Aster DEX 和 Binance 提供支持。',

    // Landing Page CTA
    readyToDefine: '准备好定义 AI 交易的未来吗？',
    startWithCrypto:
      '从加密市场起步，扩展到 TradFi。IZF 是 AgentFi 的基础架构。',
    getStartedNow: '立即开始',
    viewSourceCode: '查看源码',

    // Features Section
    coreFeatures: '核心功能',
    whyChooseIZF: '为什么选择 IZF？',
    ...
    aboutIZF: '关于 IZF',
    whatIsIZF: '什么是 IZF？',
    IZFNotAnotherBot: "IZF 不是另一个交易机器人，而是 AI 交易的 'Linux' ——",
    IZFDescription1: "一个透明、可信任的开源 OS，提供统一的 '决策-风险-执行'",
    IZFDescription2: '层，支持所有资产类别。',
    IZFDescription3:
      '从加密市场起步（24/7、高波动性完美测试场），未来扩展到股票、期货、外汇。核心：开放架构、AI',
    IZFDescription4:
      '达尔文主义（多代理自竞争、策略进化）、CodeFi 飞轮（开发者 PR',
    IZFDescription5: '贡献获积分奖励）。',
    ...
    // How It Works Section
    howToStart: '如何开始使用 IZF',
    fourSimpleSteps: '四个简单步骤，开启 AI 自动交易之旅',
    step1Title: '拉取 GitHub 仓库',
    step1Desc:
      'git clone https://github.com/IZFAiOS/IZF 并切换到 dev 分支测试新功能。',
    step2Title: '配置环境',
    step2Desc:
    ...
    riskWarningText:
      'dev 分支不稳定，勿用无法承受损失的资金。IZF 非托管，无官方策略。交易有风险，投资需谨慎。',
    ...
    // ===== 入门指南 =====
    faqWhatIsIZF: 'IZF 是什么？',
    faqWhatIsIZFAnswer:
      'IZF 是一个开源的 AI 驱动交易操作系统，支持加密货币和美股市场。它使用大语言模型（LLM）如 DeepSeek、GPT、Claude、Gemini 来分析市场数据，进行自主交易决策。核心功能包括：多 AI 模型支持、多交易所交易、可视化策略构建器、回测系统。',

    faqHowDoesItWork: 'IZF 是如何工作的？',
    faqHowDoesItWorkAnswer:
      'IZF 分 5 步工作：1）配置 AI 模型和交易所 API 凭证；2）创建交易策略（币种选择、指标、风控）；3）创建"交易员"，组合 AI 模型 + 交易所 + 策略；4）启动交易员 - 它会定期分析市场数据并做出买入/卖出/持有决策；5）在仪表板上监控表现。AI 使用思维链（Chain of Thought）推理来解释每个决策。',

    faqIsProfitable: 'IZF 能盈利吗？',
    faqIsProfitableAnswer:
      'AI 交易是实验性的，不保证盈利。加密货币期货波动性大、风险高。IZF 仅用于教育和研究目的。我们强烈建议：从小额开始（10-50 USDT），不要投入超过承受能力的资金，在实盘交易前充分回测，并理解过去的表现不代表未来的结果。',
    ...
    faqSupportedAIModelsAnswer:
      'IZF 支持 7+ 种 AI 模型：DeepSeek（推荐性价比）、阿里云通义千问、OpenAI（GPT-5.2）、Anthropic Claude、Google Gemini、xAI Grok、Kimi（月之暗面）。您也可以使用任何 OpenAI 兼容的 API 端点。每个模型各有优势 - DeepSeek 性价比高，OpenAI 能力强但贵，Claude 擅长推理。',
    ...
    // ===== 安装部署 =====
    faqHowToInstall: '如何安装 IZF？',
    faqHowToInstallAnswer:
      '最简单的方法（Linux/macOS）：运行 "curl -fsSL https://raw.githubusercontent.com/IZFAiOS/IZF/main/install.sh | bash" - 这会自动安装 Docker 容器。然后在浏览器中打开 http://127.0.0.1:3000。手动安装或开发请克隆仓库并按照 README 说明操作。',
    ...
    faqManualInstallationAnswer:
      '前置条件：Go 1.21+、Node.js 18+、TA-Lib。步骤：1）克隆仓库："git clone https://github.com/IZFAiOS/IZF.git"；2）安装后端依赖："go mod download"；3）安装前端依赖："cd web && npm install"；4）构建后端："go build -o IZF"；5）运行后端："./IZF"；6）运行前端（新终端）："cd web && npm run dev"。访问 http://127.0.0.1:3000',
    ...
    faqUpdateIZF: '如何更新 IZF？',
    faqUpdateIZFAnswer:
      'Docker 方式：运行 "docker compose pull && docker compose up -d" 拉取最新镜像并重启。手动安装：后端 "git pull && go build -o IZF"，前端 "cd web && npm install && npm run build"。data.db 中的配置在更新时会保留。',
    ...
    faqHyperliquidSetupAnswer:
      'Hyperliquid 是去中心化交易所，需要钱包认证。步骤：1）访问 app.hyperliquid.xyz；2）连接钱包；3）生成 API 钱包（推荐）或使用主钱包；4）复制钱包地址和私钥；5）在 IZF 中添加 Hyperliquid 交易所并填入凭证。无需 KYC，完全链上。',
    ...
    faqStopLossTakeProfit: 'IZF 支持止损止盈吗？',
    faqStopLossTakeProfitAnswer:
    ...
    faqMultipleTradersAnswer:
      '可以！IZF 支持运行 20+ 个并发交易员。每个交易员可以有不同的：AI 模型、交易所账户、策略、决策间隔。用于 A/B 测试策略、比较 AI 模型或跨交易所分散风险。在竞赛页面监控所有交易员。',
    ...
    faqPortInUseAnswer:
      '查看占用端口的进程：macOS/Linux 用 "lsof -i :8080"，Windows 用 "netstat -ano | findstr 8080"。终止进程或在 .env 中修改端口：IZF_BACKEND_PORT=8081、IZF_FRONTEND_PORT=3001。然后 "docker compose down && docker compose up -d" 重启。',

    faqFrontendNotLoading: '前端一直显示"加载中..."',
    faqFrontendNotLoadingAnswer:
      '后端可能未运行或无法访问。检查：1）"curl http://127.0.0.1:8080/api/health" 应返回 {"status":"ok"}；2）"docker compose ps" 验证容器运行中；3）查看后端日志："docker compose logs IZF-backend"；4）确保防火墙允许 8080 端口。',

    faqDatabaseLocked: '数据库锁定错误',
    faqDatabaseLockedAnswer:
      '多个进程同时访问 SQLite 导致。解决方案：1）停止所有进程："docker compose down" 或 "pkill IZF"；2）如有锁文件删除："rm -f data/data.db-wal data/data.db-shm"；3）重启："docker compose up -d"。只能有一个后端实例访问数据库。',

    faqTALibNotFound: '构建时找不到 TA-Lib',
    faqTALibNotFoundAnswer:
      'TA-Lib 是技术指标所需。安装：macOS："brew install ta-lib"；Ubuntu/Debian："sudo apt-get install libta-lib0-dev"；CentOS："yum install ta-lib-devel"。安装后重新构建："go build -o IZF"。Docker 镜像已预装 TA-Lib。',
    ...
    faqBinancePositionModeAnswer:
      '错误："Order\'s position side does not match user\'s setting"。您处于单向持仓模式，但 IZF 需要双向持仓模式。修复：1）先平掉所有持仓；2）币安合约 → 设置（齿轮图标）→ 偏好设置 → 持仓模式 → 切换为"双向持仓"；3）重启交易员。',

    faqBalanceShowsZero: '账户余额显示 0',
    faqBalanceShowsZeroAnswer:
      '资金可能在现货钱包而非合约钱包。解决方案：1）在币安进入钱包 → 合约 → 划转；2）将 USDT 从现货划转到合约；3）刷新 IZF 仪表板。也检查：资金未被理财/质押产品锁定。',
    ...
    faqEncryptionDetails: 'IZF 使用什么加密？',
    faqEncryptionDetailsAnswer:
      'IZF 使用多层加密：1）AES-256-GCM 用于数据库存储（API 密钥、密钥）；2）RSA-2048 用于可选的传输加密（浏览器到服务器）；3）JWT 用于认证令牌。密钥在安装时生成。HTTPS 环境启用 TRANSPORT_ENCRYPTION=true。',

    faqSecurityBestPractices: '安全最佳实践是什么？',
    faqSecurityBestPracticesAnswer:
      '建议：1）使用带 IP 白名单和最小权限（仅合约交易）的交易所 API 密钥；2）为 IZF 使用专用子账户；3）远程部署启用 TRANSPORT_ENCRYPTION；4）切勿分享 .env 或 data.db 文件；5）使用有效证书的 HTTPS；6）定期轮换 API 密钥；7）监控账户活动。',

    faqCanIZFStealFunds: 'IZF 会盗取我的资金吗？',
    faqCanIZFStealFundsAnswer:
      'IZF 是开源的（AGPL-3.0 许可）- 您可以在 GitHub 审计所有代码。API 密钥存储在您的机器本地，从不发送到外部服务器。IZF 只有您通过 API 密钥授予的权限。为最大安全：使用仅交易权限（无提现）的 API 密钥，启用 IP 白名单，使用专用子账户。',
    ...
    faqCustomAIAPIAnswer:
      '可以！IZF 支持任何 OpenAI 兼容的 API。在配置 → AI 模型 → 自定义 API 中：1）输入 API 端点 URL（如 https://your-api.com/v1）；2）输入 API 密钥；3）指定模型名称。适用于自托管模型、替代提供商或通过第三方代理的 Claude。',

    faqAIHallucinations: 'AI 幻觉问题怎么办？',
    faqAIHallucinationsAnswer:
      'AI 模型有时会产生不正确或虚构的信息（"幻觉"）。IZF 通过以下方式缓解：1）提供带真实市场数据的结构化提示词；2）强制 JSON 输出格式；3）执行前验证订单。但 AI 交易是实验性的 - 始终监控决策，不要完全依赖 AI 判断。',
    ...
    // ===== 参与贡献 =====
    faqHowToContribute: '如何为 IZF 做贡献？',
    faqHowToContributeAnswer:
      'IZF 是开源项目，欢迎贡献！贡献方式：1）代码 - 修复 bug、添加功能（查看 GitHub Issues）；2）文档 - 改进指南、翻译；3）Bug 报告 - 详细报告问题；4）功能建议 - 提出改进意见。从标记为"good first issue"的问题开始。所有贡献者可能获得空投奖励。',

    faqPRGuidelines: 'PR 指南是什么？',
    faqPRGuidelinesAnswer:
      'PR 流程：1）Fork 仓库到您的账户；2）从 dev 创建功能分支："git checkout -b feat/your-feature"；3）修改代码，运行 lint："npm --prefix web run lint"；4）使用 Conventional Commits 格式提交；5）推送并创建 PR 到 IZFAiOS/IZF:dev；6）关联相关 issue（Closes #123）；7）等待审核。保持 PR 小而聚焦。',
    ...
    environmentCheck: {
      ...
      unsupportedDesc:
        '请通过 HTTPS 或本机 localhost 访问 IZF，并避免嵌入不安全 iframe/反向代理，以符合浏览器的 Web Crypto 规则。',
  ...
  id: {
    ...
    // Footer
    footerTitle: 'IZF - Sistem Trading AI',
    ...
    heroTitle1: 'Read the Market.',
    heroTitle2: 'Write the Trade.',
    heroDescription: 'IZF adalah standar masa depan untuk trading AI — OS trading agensi yang terbuka dan didorong komunitas. Mendukung Binance, Aster DEX dan bursa lainnya, self-hosted, kompetisi multi-agen, biarkan AI secara otomatis membuat keputusan, mengeksekusi dan mengoptimalkan trading untuk Anda.',
    poweredBy: 'Didukung oleh Aster DEX dan Binance.',
    readyToDefine: 'Siap mendefinisikan masa depan trading AI?',
    startWithCrypto: 'Dimulai dari pasar kripto, berkembang ke TradFi. IZF adalah infrastruktur AgentFi.',
    getStartedNow: 'Mulai Sekarang',
    viewSourceCode: 'Lihat Kode Sumber',
    coreFeatures: 'Fitur Inti',
    whyChooseIZF: 'Mengapa Memilih IZF?',
    ...
    aboutIZF: 'Tentang IZF',
    whatIsIZF: 'Apa itu IZF?',
    IZFNotAnotherBot: "IZF bukan bot trading biasa, melainkan 'Linux' dari trading AI —",
    IZFDescription1: "OS open source yang transparan dan terpercaya yang menyediakan lapisan",
    IZFDescription2: "'keputusan-risiko-eksekusi' terpadu, mendukung semua kelas aset.",
    IZFDescription3: 'Dimulai dari pasar kripto (24/7, volatilitas tinggi sebagai tempat uji sempurna), ekspansi masa depan ke saham, futures, forex. Inti: arsitektur terbuka, AI',
    IZFDescription4: 'Darwinisme (kompetisi mandiri multi-agen, evolusi strategi), flywheel CodeFi',
    IZFDescription5: '(pengembang mendapat reward poin untuk kontribusi PR).',
    ...
    step1Desc: 'git clone https://github.com/IZFAiOS/IZF dan beralih ke branch dev untuk menguji fitur baru.',
    ...
    riskWarningText: 'Branch dev tidak stabil, jangan gunakan dana yang tidak sanggup Anda rugi. IZF non-custodial, tanpa strategi resmi. Trading memiliki risiko, investasi dengan hati-hati.',
    ...
    accessIZFPlatform: 'Akses Platform IZF',
    ...
    faqSubtitle: 'Temukan jawaban untuk pertanyaan umum tentang IZF',
    ...
    faqWhatIsIZF: 'Apa itu IZF?',
    faqWhatIsIZFAnswer: 'IZF adalah sistem operasi trading bertenaga AI open-source untuk pasar kripto dan saham AS. Ia menggunakan model bahasa besar (LLM) seperti DeepSeek, GPT, Claude, Gemini untuk menganalisis data pasar dan membuat keputusan trading secara otonom. Fitur utama: dukungan multi-model AI, trading multi-bursa, dan pembangun strategi visual.',
    faqHowDoesItWork: 'Bagaimana cara kerja IZF?',
    faqHowDoesItWorkAnswer: 'IZF bekerja dalam 5 langkah: 1) Konfigurasi model AI dan kredensial API bursa; 2) Buat strategi trading (pemilihan koin, indikator, kontrol risiko); 3) Buat "Trader" menggabungkan Model AI + Bursa + Strategi; 4) Mulai trader - dia akan menganalisis data pasar secara berkala dan membuat keputusan beli/jual/tahan; 5) Pantau performa di dasbor.',
    faqIsProfitable: 'Apakah IZF menguntungkan?',
    faqIsProfitableAnswer: 'Trading AI bersifat eksperimental dan TIDAK dijamin menguntungkan. Futures kripto sangat volatil dan berisiko. IZF dirancang untuk tujuan edukasi dan riset. Kami sangat menyarankan: mulai dengan jumlah kecil (10-50 USDT), jangan investasi melebihi yang sanggup Anda rugi, uji sebelum trading nyata.',
    ...
    faqSupportedAIModelsAnswer: 'IZF mendukung 7+ model AI: DeepSeek (direkomendasikan untuk biaya/performa), Qwen, OpenAI (GPT), Claude, Gemini, Grok, dan Kimi. Anda juga dapat menggunakan endpoint API yang kompatibel dengan OpenAI.',
    ...
    faqHowToInstall: 'Bagaimana cara menginstal IZF?',
    faqHowToInstallAnswer: 'Metode termudah (Linux/macOS): Jalankan "curl -fsSL https://raw.githubusercontent.com/IZFAiOS/IZF/main/install.sh | bash" - ini menginstal kontainer Docker secara otomatis. Lalu buka http://127.0.0.1:3000 di browser Anda.',
    ...
    faqManualInstallationAnswer: 'Prasyarat: Go 1.21+, Node.js 18+, TA-Lib. Langkah: 1) Clone repo; 2) "go mod download"; 3) "cd web && npm install"; 4) "go build -o IZF"; 5) "./IZF"; 6) "cd web && npm run dev".',
    ...
    faqUpdateIZF: 'Bagaimana cara memperbarui IZF?',
    faqUpdateIZFAnswer: 'Docker: "docker compose pull && docker compose up -d". Manual: "git pull && go build -o IZF" untuk backend, "cd web && npm install && npm run build" untuk frontend.',
    ...
    faqHyperliquidSetupAnswer: 'Hyperliquid adalah bursa terdesentralisasi. Langkah: 1) Kunjungi app.hyperliquid.xyz; 2) Hubungkan wallet; 3) Buat API wallet; 4) Salin alamat dan private key; 5) Tambahkan di IZF. Tanpa KYC.',
    ...
    faqStopLossTakeProfit: 'Apakah IZF mendukung stop-loss dan take-profit?',
    ...
    faqMultipleTradersAnswer: 'Ya! IZF mendukung 20+ trader bersamaan. Gunakan untuk A/B test strategi, bandingkan model AI, atau diversifikasi lintas bursa.',
    ...
    faqPortInUseAnswer: 'Periksa proses yang menggunakan port. Ubah port di .env: IZF_BACKEND_PORT=8081, IZF_FRONTEND_PORT=3001.',
    ...
    faqBinancePositionModeAnswer: 'Anda dalam mode One-way tetapi IZF memerlukan Hedge Mode. Tutup semua posisi, beralih ke Hedge Mode, restart trader.',
    ...
    faqEncryptionDetails: 'Enkripsi apa yang digunakan IZF?',
    faqEncryptionDetailsAnswer: 'IZF menggunakan: 1) AES-256-GCM untuk penyimpanan database; 2) RSA-2048 untuk enkripsi transport opsional; 3) JWT untuk token autentikasi.',
    ...
    faqCanIZFStealFunds: 'Bisakah IZF mencuri dana saya?',
    faqCanIZFStealFundsAnswer: 'IZF open-source (lisensi AGPL-3.0) - Anda bisa audit semua kode. API key disimpan lokal di mesin ANDA, tidak pernah dikirim ke server eksternal.',
    ...
    faqCustomAIAPIAnswer: 'Ya! IZF mendukung API yang kompatibel dengan OpenAI. Masukkan URL endpoint, API key, dan nama model.',
    faqAIHallucinations: 'Bagaimana dengan halusinasi AI?',
    faqAIHallucinationsAnswer: 'IZF memitigasi dengan: prompt terstruktur, format output JSON, dan validasi order sebelum eksekusi. Namun trading AI tetap eksperimental.',
    ...
    faqHowToContribute: 'Bagaimana cara berkontribusi ke IZF?',
    faqHowToContributeAnswer: 'IZF open-source dan menyambut kontribusi! Cara: 1) Kode - perbaiki bug, tambah fitur; 2) Dokumentasi; 3) Laporan Bug; 4) Ide Fitur. Semua kontributor mungkin mendapat reward airdrop.',
    faqPRGuidelines: 'Apa panduan PR?',
    faqPRGuidelinesAnswer: 'Proses PR: 1) Fork repo; 2) Buat branch fitur dari dev; 3) Buat perubahan, jalankan lint; 4) Commit dengan format Conventional Commits; 5) Push dan buat PR ke IZFAiOS/IZF:dev.',
    ...
    environmentCheck: {
      ...
      unsupportedDesc: 'Buka IZF melalui HTTPS (atau http://localhost saat pengembangan).',
